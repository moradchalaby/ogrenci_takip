@section('script')
    ...
    <script>
        ...
        $('#kasaadd').on("submit", function(e) {
            e.preventDefault();

            var formdata = $('#kasaadd').serializeArray();
            console.log(formdata);
            $.ajax({

                url: "{{ route( 'kasa.store') }}",
                type: 'POST',
                data: formdata,
                dataType: 'text',
                success: (data) => {
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: dat["tutar"] + dat["kur"] +'<br>  İşlem Başarılı <br>',
                    })

                    document.getElementById("kasaadd").reset();
                },
                error: function(data) {
                    // var dat = JSON.parse(data);
                    $('#modalKasaAdd').modal('hide');
                    console.log('error');
                    console.log(data);
...
<!--    SHOW AJAX-->
    ...
    <script>
        ...
        function odemeturgetir(array, veri) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
    ...
    <script>
        ...
        function odemesekligetir(array, veri) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
    ...
    <script>
        $( document ).ready(function() {
            odemeturgetir(['#modalAdd #odenen','#modalKasaAdd #odenen','#modalEdit #tur'],'SADAKA');
            odemesekligetir(['#modalAdd #odemeSekli','#modalKasaAdd #odemeSekli','#modalEdit #odeme_sekli'],'NAKİT');
            kurgetir(['#modalAdd #kur','#modalEdit #kur','#modalKasaAdd #kur'],'₺');
